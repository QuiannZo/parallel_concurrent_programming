...
typedef struct shared_data {
  ...
  size_t position;
  pthread_mutex_t position_mutex;  // Mutex for the pos variable.
} shared_data_t;
...
int analyze_arguments(int argc, char* argv[], shared_data_t* shared_data) {
  if (argc == 4) {
    ...
    if (sscanf(argv[2], "%" SCNu64, &shared_data->stage1_duration) != 1) {
      fprintf(stderr, "invalid stage 1 duration: %s\n", argv[2]);
      return 12;
    }

    if (sscanf(argv[3], "%" SCNu64, &shared_data->stage2_duration) != 1) {
...
int create_threads(shared_data_t* shared_data) {
  ...
  if (threads && private_data) {
    for (size_t index = 0; index < shared_data->team_count; ++index) {
      ...
      error = pthread_create(&threads[index], NULL, start_race, &private_data[index]);
    ...
    for (size_t index = shared_data->team_count; index < thread_count; ++index) {
      ...
      error = pthread_create(&threads[index], NULL, finish_race, &private_data[index]);
...
void* start_race(void* data) {
  ...
  usleep(shared_data->stage1_duration);
...
void* finish_race(void* data) {
  ...
  const size_t team_number = private_data->thread_number - shared_data->team_count;
  assert(team_number < shared_data->team_count);

  usleep(shared_data->stage2_duration);
