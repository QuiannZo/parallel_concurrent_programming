...
int main(int argc, char* argv[]) {
    ...
    // create thread_count as a result of converting argv[1] to an integer
    ...
    if (shared_data) {
        ...
        for (uint64_t thread_number = 0; thread_number < shared_data->thread_count; ++thread_number) {
            ...
            //
            error = sem_init(&shared_data->can_greet[thread_number], /*pshared*/ 0, 0);
        }
        ...
        if (shared_data->can_greet) {
            ...
            // Increment the semaphore of the first thread
...
void* greet(void* data) {
    ...
    printf("Hello from the secondary thread %" PRIu64 " of %" PRIu64 "\n", private_data->thread_number, shared_data->thread_count);

    // Allow the subsequent thread to do the task
